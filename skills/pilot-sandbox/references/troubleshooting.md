...
| Symptom | Cause | Fix |
|---|---|---|
| Pilot commands fail while `pilotctl --json info` works; `proxy CONNECT ...: 407 Proxy Authentication Required` (or `malformed HTTP status code`) in `daemon.log` | wrong, missing or expired proxy credentials. Muse rotates them every few minutes, and a running daemon keeps the ones it started with | rerun `pilot-up.sh` from a new shell (current credentials): it restarts whatever was started with other proxy settings, and an online node its respawn loop runs whose log shows 407s since the loop last started it. A node started another way (by hand, `pilotctl daemon start`, a service) only gets a note: restart it the way it was started, or `pilot-up.sh --stop && pilot-up.sh`. Elsewhere, check the `user:pass@` part of `HTTPS_PROXY`; URL-encode `@`, `:` and `/` inside the password |
