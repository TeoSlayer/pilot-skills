...
| `127.0.0.1:3128 is taken by a process that is not egress_relay.py` | something else listens on the relay port | `PILOT_RELAY_LISTEN=127.0.0.1:<free port>`. While that node runs, reruns keep its relay on that port without the variable. An `egress_relay.py` started by hand is used as is (and never stopped) |
| `this shell's proxy (HTTPS_PROXY) is on 127.0.0.1:3128, where the egress relay would listen` | the real proxy (with credentials) listens on the relay's default address, as squid does | `PILOT_RELAY_LISTEN=127.0.0.1:<free port>`. Only a daemon without `-proxy-cmd` needs the relay |
| `this shell's HTTPS_PROXY is the egress relay's address` | the shell exported the relay's URL without credentials (the manual recipe does), and no relay runs | rerun from a new shell with the sandbox's own `HTTPS_PROXY`; `pilot-up.sh` starts the relay itself |
| `denied ...: no valid relay token` in `~/.pilot/egress_relay.log` | a local process without the relay token tried to use pilot-up's relay (it serves only the node's daemon and SNI router, so other users cannot borrow the proxy credentials) | nothing, unless `pilot-up.sh` reports that the node itself was refused: then `pilot-up.sh --stop && pilot-up.sh` (for example after `~/.pilot/egress_relay.token` was deleted) |
...
| the install one-liner prints only `curl: (22) ...` or `curl: (56) CONNECT tunnel failed, response 407` | the proxy rejected the credentials this shell has (Muse rotates them every few minutes) | rerun the same command from a fresh shell. `muse/install.sh` names a 407 on its own downloads |
...
| `pilot-up.sh` exits 3 | older daemon (v1.13.10 and earlier have no `-proxy`), and no root / CAP_SYS_ADMIN for the fallback | once a release whose `pilot-daemon -h` lists `-proxy` is out (pilot-up says when the installed one already is the latest): `curl -fsSL https://raw.githubusercontent.com/TeoSlayer/pilot-skills/main/muse/install.sh \| PILOT_UPGRADE=1 bash` (works as root: it passes `PILOT_ALLOW_ROOT=1` to the official installer, and restarts the node). Or rerun as root with `sudo -E` where root has CAP_SYS_ADMIN |
